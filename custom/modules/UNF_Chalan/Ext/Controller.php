...
class ChalanController extends SugarController
{
    public function action_getDetails()
    {
        $return = [];

        $leadName = $_REQUEST['productId'];
...
ON 
	L.id = LC.id_c
WHERE
    L.deleted = 0 AND REPLACE(L.name, ' ', '') = REPLACE('{$lastName}', ' ', '') AND  LC.lead_source_type_c = '$sourceType' AND L.id != '$leadName'";
