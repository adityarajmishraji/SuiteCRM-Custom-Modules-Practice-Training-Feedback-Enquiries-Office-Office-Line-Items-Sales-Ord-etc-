<?php
$liModuleName = $this->taxTableModule;
$liGroupModuleName = $this->taxTableGroup;
$relACol = $this->taxTableRelACol;

// $GLOBALS['log']->code(print_r($this, true));
// $GLOBALS['log']->code($_POST);

if (empty($this->id) || (isset($_POST['duplicateSave']) && $_POST['duplicateSave'] == 'true')) {
    if (isset($_POST['pm_line_item_id'])) {
        unset($_POST['pm_line_item_id']);
    }

    if (isset($_POST['pm_group_id'])) {
        unset($_POST['pm_group_id']);
    }
}
...
for ($i = 0; $i < $line_count; ++$i) {
    if (isset($_POST[$key . 'deleted'][$i]) && $_POST[$key . 'deleted'][$i] == 1) {
        ...
        // $GLOBALS['log']->code(print_r($lineItem, true));
