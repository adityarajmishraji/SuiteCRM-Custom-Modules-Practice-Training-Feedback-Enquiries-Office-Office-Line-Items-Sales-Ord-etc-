// Add Pending button at Line Item Subpanel top
$(document).ready(function() {
    if (!$('#pendingBtn').length) {
        if ($('#lineItems').length) {
            $('#lineItems').before('<div style="text-align: left; margin: 10px 0;"><button id="pendingBtn" onclick="openPendingPopup()" style="background: #007bff; color: white; padding: 8px 16px; border: none; border-radius: 4px;">Pending</button></div>');
        }
    }
});
...
function addLineItem(grpNo) {
    ...
    // Quantity
    a = headerRow.insertCell(colIndexNo);
    a.innerHTML = "<input type='text' name='line_item_qty[" + rowNo + "]' id='line_item_qty" + rowNo + "' maxlength='255' value='1' class='calu input-number cls_qty' autocomplete='off' >";
    colIndexNo++;
    ...
    // Amount
    a = headerRow.insertCell(colIndexNo);
    a.innerHTML = "<input type='text' name='line_item_amount[" + rowNo + "]' id='line_item_amount" + rowNo + "' maxlength='255' value='' class='calu input-number cls_amount' autocomplete='off' >";
    colIndexNo++;
...
function openProductPopup(ln) {
    ...
    var popupRequestData = {
        ...
        "form_name": "EditView",
        "field_to_name_array": {
            "id": "line_item_unf_chalan_id_c" + ln,
...
function openPendingPopup(ln) {
    lineno = ln;
    var popupRequestData = {
        "call_back_function": "setPendingReturn",
        "form_name": "EditView",
        "field_to_name_array": {
...
function setPendingReturn(popupReplyData) {
    set_return(popupReplyData);
    getPendingDetails(lineno);
}

function getPendingDetails(ln) {
    var productId = $('#line_item_unf_sales_order_id_c' + ln).val();
    if (productId != '') {
        $.ajax({
            url: "index.php?module=AOS_Products&action=getDetails",
            type: "GET",
            ...
            success: function (result) {
                var recordDetails = JSON.parse(result);
                $('#line_item_name' + ln).val(recordDetails.name);
                $('#line_item_uom' + ln).val(recordDetails.uom_c);
                $('#line_item_rate' + ln).val(recordDetails.price);
            }
        });
    }
